rankhospital <- function(state, outcome, num = "best") {
    ...
    retval <- NULL
    outlen <- length(outcome.df[[hospital.name.colname]])
    if (is.character(num) && tolower(num) == "best") {
        retval <- outcome.df[[hospital.name.colname]][1]
    }
    else if (is.character(num) && tolower(num) == "worst") {
        retval <- outcome.df[[hospital.name.colname]][outlen]
    }
        ...
        retval <- outcome.df[[hospital.name.colname]][num]
    }
